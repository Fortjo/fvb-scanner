...
def check_secrets_present():
    """[DIAGNOSTIC] Prints only the LENGTH of each env var — never the
    actual value — to definitively confirm whether the secrets are truly
    reaching Python empty, or whether GitHub was just blanking them in the
    log display while they were actually set correctly all along."""
    for name in ["EMAIL_ADDRESS", "EMAIL_PASSWORD", "EMAIL_TO"]:
        val = os.environ.get(name, "")
        print(f"[secret check] {name}: length={len(val)}, "
              f"first_char={val[0] if val else '(empty)'}")
...
def build_email_body(fresh_weeks: int = 12) -> str:
    ...
    fresh = longs[longs["fresh"]].copy()

    # [SORT] "high probability" isn't one number — it's track record AND
    # whether the setup still has room left. A 90%-win-rate ticker that's
    # already run most of the way to its exit (low risk_reward, per the
    # PKG/NDAQ lesson) isn't as good a CURRENT opportunity as one with the
    # same track record and room still to run. So: rating tier first
    # (Strong > Moderate > Mixed > Weak > Thin sample), then risk_reward
    # descending within tier (NaN/below-stop-level sorts to the bottom —
    # those need a manual chart check, not a top spot), then win rate as
    # the final tiebreaker.
    rating_rank = {"Strong track record": 0, "Moderate track record": 1,
                   "Mixed track record": 2, "Weak track record": 3,
                   "Thin sample (n<2)": 4}
    fresh["_rating_rank"] = fresh["rating"].map(rating_rank).fillna(5)
    fresh["_rr_sort"] = fresh["risk_reward"].fillna(-999)  # NaN/below-stop -> sorts last
    fresh = fresh.sort_values(["_rating_rank", "_rr_sort", "hist_win_rate"],
                              ascending=[True, False, False])

    lines = []
    lines.append(f"Weekly 33FVB scan — {len(df)} tickers checked, {len(fresh)} fresh buy signals\n")
    lines.append("Sorted by: track record tier, then remaining risk/reward, then win rate.\n")
    lines.append("=" * 70)
