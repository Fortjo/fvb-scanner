...
def run(df: pd.DataFrame, band: str, exit_mode: str) -> list[dict]:
    ...
    trades = []
    in_pos, entry_px, entry_i, entry_tp_target = False, np.nan, -1, np.nan
    ...
    for i in range(len(df)):
        ...
        if in_pos and i > entry_i:
            ...
            if exit_mode == "NBarHigh":
                # [TP FIX — July 2026] entry_tp_target is fixed at entry
                # (see below), not the live-recomputed prev_hh — see the
                # comment at the entry block for why.
                if not np.isnan(entry_tp_target) and hgh[i] >= entry_tp_target:
                    exit_px, reason = max(o[i], entry_tp_target), "TP"
        ...
        if (not in_pos) and green[i] == True and not np.isnan(band_px) \
            ...
            # [TP FIX — July 2026] prev_hh (200-week rolling high) can lock
            # onto an ancient, no-longer-relevant spike (confirmed on real
            # tickers: RDW's 200wk high sat at $26.66 from early SPAC-era
            # trading while the stock actually traded $7-13 — TP became
            # mathematically unreachable), or simply be NaN if the stock
            # hasn't existed for a full 200 bars yet (LUNR). Fixed at
            # ENTRY, not recomputed each bar. Validated across the full
            # universe: raised expectancy from +14.72% to +16.01%/trade
            # and PF from 3.37 to 3.57 vs always using the raw 200-bar
            # high, while fixing both dead cases — full backtest, not a
            # guess. Mirrors the same fix in the Pine indicator.
            hh_missing = np.isnan(prev_hh[i])
            hh_stale = (not hh_missing) and (prev_hh[i] > entry_px * 2.5)
            if (hh_missing or hh_stale) and i > 0:
                past_pivots = piv[:i]
                valid = past_pivots[~np.isnan(past_pivots)]
                # [FIX] the fallback must be a real TARGET — i.e. ABOVE
                # entry — not just "the most recent confirmed pivot,
                # whatever its price." A pivot from before this uptrend
                # started can sit BELOW entry_px, which would make TP
                # register as already-hit the instant the trade opens.
                above_entry = valid[valid > entry_px]
                if len(above_entry) > 0:
                    entry_tp_target = above_entry[-1]
                else:
                    entry_tp_target = prev_hh[i]  # even a stale/far value beats a bogus one below entry
            else:
                entry_tp_target = prev_hh[i]
    return trades
